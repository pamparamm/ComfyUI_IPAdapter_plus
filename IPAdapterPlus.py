...
def ipadapter_execute(
    ...
    is_kwai_kolors_faceid = (
        ...
        and ipadapter["image_proj"]["perceiver_resampler.layers.0.0.to_out.weight"].shape[0] == 4096
    )
    ...
    if layer_weights is not None and layer_weights != "":
        ...
        weight_type = (
            weight_type if weight_type == "style transfer precise" or weight_type == "composition precise" else "linear"
        )
    ...
    if image is not None:
        ...
        if is_plus:
            ...
            image_negative = (
                image_negative if image_negative is not None else torch.zeros([1, clipvision_size, clipvision_size, 3])
            )
            img_uncond_embeds = encode_image_masked(
                clipvision, image_negative, batch_size=encode_batch_size, clipvision_size=clipvision_size
        ...
        if neg_embed is not None:
            ...
            if is_plus:
                img_uncond_embeds = encode_image_masked(
                    clipvision, torch.zeros([1, clipvision_size, clipvision_size, 3]), clipvision_size=clipvision_size
                ).penultimate_hidden_states
            else:
                img_uncond_embeds = torch.zeros_like(img_cond_embeds)
    ...
    if combine_embeds != "concat" and img_cond_embeds.shape[0] > 1 and not unfold_batch:
        if combine_embeds == "add":
            ...
            img_cond_embeds = torch.mean(
                img_cond_embeds / torch.norm(img_cond_embeds, dim=0, keepdim=True), dim=0
            ).unsqueeze(0)
            if face_cond_embeds is not None:
                face_cond_embeds = torch.mean(
                    face_cond_embeds / torch.norm(face_cond_embeds, dim=0, keepdim=True), dim=0
                ).unsqueeze(0)
            if img_comp_cond_embeds is not None:
                img_comp_cond_embeds = torch.mean(
                    img_comp_cond_embeds / torch.norm(img_comp_cond_embeds, dim=0, keepdim=True), dim=0
                ).unsqueeze(0)
    ...
    if is_faceid and is_plus:
        cond = ipa.get_image_embeds_faceid_plus(
            face_cond_embeds, img_cond_embeds, weight_faceidv2, is_faceidv2, encode_batch_size
        )
        # TODO: check if noise helps with the uncond face embeds
        uncond = ipa.get_image_embeds_faceid_plus(
            torch.zeros_like(face_cond_embeds), img_uncond_embeds, weight_faceidv2, is_faceidv2, encode_batch_size
        )
    else:
...
class IPAdapterUnifiedLoader:
    ...
    def load_models(self, model, preset, lora_strength=0.0, provider="CPU", ipadapter=None):
        ...
        return (
            model,
            {"clipvision": self.clipvision, "ipadapter": self.ipadapter, "insightface": self.insightface},
        )
...
class IPAdapterInsightFaceLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {"provider": (["CPU", "CUDA", "ROCM"],), "model_name": (["buffalo_l", "antelopev2"],)},
        }
...
class IPAdapterSimple:
    ...
    def apply_ipadapter(self, model, ipadapter, image, weight, start_at, end_at, weight_type, attn_mask=None):
        ...
        return ipadapter_execute(
            model.clone(), ipadapter["ipadapter"]["model"], ipadapter["clipvision"]["model"], **ipa_args
        )
    ...
    def apply_ipadapter(
        ...
        for i in range(len(image)):
            ...
            ipa_args = {
                ...
                else ipadapter["insightface"]["model"]
...
class IPAdapterEncoder:
    ...
    def encode(self, ipadapter, image, weight, mask=None, clip_vision=None):
        ...
        if is_plus:
            img_cond_embeds = img_cond_embeds.penultimate_hidden_states
            img_uncond_embeds = encode_image_masked(
                clip_vision, torch.zeros([1, clipvision_size, clipvision_size, 3]), clipvision_size=clipvision_size
            ).penultimate_hidden_states
        else:
...
class IPAdapterSaveEmbeds:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {"embeds": ("EMBEDS",), "filename_prefix": ("STRING", {"default": "IP_embeds"})},
        }
...
class IPAdapterRegionalConditioning:
    ...
    def conditioning(
        self, image, image_weight, prompt_weight, weight_type, start_at, end_at, mask=None, positive=None, negative=None
    ):
        ...
        if mask is not None:
            if positive is not None:
                positive = conditioning_set_values(
                    positive, {"mask": mask, "set_area_to_bounds": set_area_to_bounds, "mask_strength": prompt_weight}
                )
            if negative is not None:
                negative = conditioning_set_values(
                    negative, {"mask": mask, "set_area_to_bounds": set_area_to_bounds, "mask_strength": prompt_weight}
                )
...
# Adapted from https://github.com/vahlok-alunmid/ComfyUI-ExtendIPAdapterClipVision by vahlok-alunmid
# Licensed under GPL-3.0
class IPAdapterExtendClipVisionInputSize:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
    ...
    def apply_patch(self, clip_vision, target_size):
        ...
        pe = torch.nn.Embedding(pe.shape[0], pe.shape[1], _weight=pe, dtype=original_dtype, device=original_device)
...
NODE_CLASS_MAPPINGS = {
    ...
    "IPAdapterExtendClipVisionInputSize": IPAdapterExtendClipVisionInputSize,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "IPAdapterExtendClipVisionInputSize": "IPAdapter Extend Clip Vision Input Size",
}
