...
def split_tiles(embeds, num_split):
    ...
    for x in embeds:
        ...
        x_split = torch.cat(
            [x[:, i * h : (i + 1) * h, j * w : (j + 1) * w, :] for i in range(num_split) for j in range(num_split)],
            dim=0,
        )
        out.append(x_split)
...
def merge_hiddenstates(x, tiles):
    ...
    for embeds in x:
        ...
        merged = torch.cat(
            [torch.cat([reshaped[i, j] for j in range(grid_size)], dim=1) for i in range(grid_size)], dim=0
        )

        merged = merged.unsqueeze(0)  # Shape: [1, grid_size*tile_size, grid_size*tile_size, embeds[-1]]

        # Pool to original size
        pooled = torch.nn.functional.adaptive_avg_pool2d(merged.permute(0, 3, 1, 2), (tile_size, tile_size)).permute(
            0, 2, 3, 1
        )
...
def merge_embeddings(x, tiles):  # TODO: rewrite
    chunk_size = tiles * tiles
    x = x.split(chunk_size)
    ...
    for embeds in x:
        num_tiles = embeds.shape[0]
        mean = torch.mean(embeds, dim=0).unsqueeze(0)
        out.append(mean)
        continue

        grid_size = int(num_tiles**0.5)
        tile_size = int(embeds.shape[1] ** 0.5)
        reshaped = embeds.reshape(grid_size, grid_size, -1)
